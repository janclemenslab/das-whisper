...
def download_data(data_path, local_data_path: Union[str, bytes, None, os.PathLike]):
    ...
    if os.path.exists(data_path):
        ...
        return str(local_data_path) + "/train"
...
def load_audio_and_label(
    ...
    for count, (audio_path, label_path) in enumerate(zip(audio_path_list, label_path_list)):
        ...
        label.update({"onset": onset_arr, "offset": offset_arr, "cluster_id": cluster_id_arr})
        local_label_list.append(label)

        if count % 10 == 0:
            progress = count / len(audio_path_list)
            print(
                "|%s%s|progress: %.2f %%" % ("-" * (int(progress * 20)), " " * (20 - int(progress * 20)), progress * 100),
                end="\r",
                flush=True,
            )
...
def load_data(audio_path_list, label_path_list, cluster_codebook=None, n_threads=1, default_config={}, ignore_cluster=False):
    ...
    for thread_id, offset in enumerate(range(0, len(audio_path_list), samples_per_thread)):
        t = threading.Thread(
            target=load_audio_and_label,
            args=(
                audio_path_list[offset : offset + samples_per_thread],
