...
def load_model(initial_model_path, total_spec_columns, dropout=0.0):
    ...
    with torch.no_grad():
        model.model.encoder.embed_positions.weight = torch.nn.Parameter(model.model.encoder.embed_positions.weight[: model.config.max_source_positions, :])
    ...
    def generate_segment_text(
        ...
        for pos in range(0, len(sliced_audios_features), batch_size):
            ...
            generated_ids = self.model.generate(
                ...
                decoder_input_ids=torch.LongTensor([self.tokenizer.convert_tokens_to_ids(["<|startoftranscript|>", "<|en|>", "<|notimestamps|>"]) for _ in range(input_features.size(0))]).to(
                    self.device
                ),
...
class WhisperSegmenter(SegmenterBase):
    def __init__(
        ...
        if device == "cuda" or device == "mps":
            self.device_list = [torch.device(device, gpu) for gpu in device_ids]
    ...
    def generate_segment_text_core(
        ...
        for pos in range(0, len(sliced_audios_features), batch_size):
            ...
            generated_ids = model.generate(
                input_features=input_features,
                decoder_input_ids=torch.LongTensor([tokenizer.convert_tokens_to_ids(["<|startoftranscript|>", "<|en|>", "<|notimestamps|>"]) for _ in range(input_features.size(0))]).to(device),
