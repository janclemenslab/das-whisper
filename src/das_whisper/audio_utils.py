...
def get_sampling_rate(file_path):
    return librosa.get_samplerate(path=file_path)
...
class SpecViewer:
    def __init__(
        ...
        self.colors = [
            np.array(mcolors.hex2color(color_string))
            for color_string in list(mcolors.TABLEAU_COLORS.values()) + list(mcolors.CSS4_COLORS.values())
        ][1:]  # Skip the first color since it looks not so good ...
        unique_colors = None
    ...
    def plot_spec_and_labels(
        ...
        for pos in range(len(label_chunked["onset"])):
            ...
            if (
                pos + 1 < len(label_chunked["onset"])
                and offset_idx == int(np.round(label_chunked["onset"][pos + 1] / spec_time_step))
                and cluster == label_chunked["cluster"][pos + 1]
            ):
                offset_idx -= 1
        ...
        for pos in range(len(prediction_chunked["onset"])):
            ...
            if (
                pos + 1 < len(prediction_chunked["onset"])
                and offset_idx == int(np.round(prediction_chunked["onset"][pos + 1] / spec_time_step))
                and cluster == prediction_chunked["cluster"][pos + 1]
            ):
                offset_idx -= 1
    ...
    def visualize(
        ...
        audio_file_name="",
        window_size=5.0,
        xticks_step_size=0.5,
        spec_width=1000,
    ):
        feature_extractor = WhisperSegFeatureExtractor(
            sr, window_size / spec_width, min_frequency, max_frequency, chunk_length=max(30, int(np.ceil(window_size)))
        )
